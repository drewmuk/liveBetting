...
pbp22 <- formatPBP(2022)
pbp21 <- formatPBP(2021)
pbp20 <- formatPBP(2020)
pbp19 <- formatPBP(2019)

masterfile <- rbind(pbp22, pbp21, pbp20, pbp19)
...
write.csv(summary(MOVfit)$coefficients[,1],'C:/Users/drewm/Desktop/liveBetting/nfl/MOVreg.csv')
write.csv(summary(OUfit)$coefficients[,1],'C:/Users/drewm/Desktop/liveBetting/nfl/OUreg.csv')
...
allgames21 <- pbp21[!duplicated(pbp21$game_id),]
allgames21$home_win <- ifelse(allgames21$home_score>allgames21$away_score,1,0)
allgames21 <- allgames21[,c('away_team','home_team','spread_line','home_win')]
write.csv(allgames21,'C:/Users/drewm/Desktop/liveBetting/nfl/allgames21.csv',row.names = FALSE)
